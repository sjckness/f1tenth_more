...
2. **`handle_obstacle`**: `IsObstacleDetected` → `Stop`. Unconditional — the sole obstacle-stop mechanism (`safety_stop_controller`, a second, since-retired mechanism, used to also exist — see "Known limitations").
...
| `IsProximityTooClose` | `/perception/front_distance`, `/scan` | Raw-sensor last resort, YOLO-independent. See `stack_params.yaml`'s `car_radius` comment for its two thresholds' derivation. |
| `IsSystemOverheated` | `/diagnostics/system_status` | Shared CPU/GPU temp+load thresholds (`sys_obs_max_temp_c`/`sys_obs_max_load_percent`). |
...
| `Stop` | — (publisher only) | Zero-speed `AckermannDriveStamped` onto the mux's `safety_stop` lane every tick; reused by both `emergency` and `handle_obstacle`. |
